    ...
    <section className="page-wrap pt-4">
      <div className="relative isolate overflow-hidden rounded-[40px] bg-brand text-white">
        ...
        <img
          src={heroCar}
          alt=""
          className="pointer-events-none absolute hidden object-contain object-center lg:block"
          style={{
            left: "26.58%",
            top: "36.82%",
            width: "53.86%",
        ...
        <div className="relative z-10 flex flex-col gap-10 px-6 py-12 sm:px-10 lg:h-[660px] lg:flex-row lg:items-start lg:justify-between lg:px-[72px] lg:py-[78px]">
          <div className="max-w-[686px] lg:pt-[90px]">
            <h1 className="text-[40px] font-bold leading-[1.16] sm:text-[50px] sm:leading-[1.45]">
              Experience the road{" "}
              <span className="lg:block">like never before</span>
            </h1>
            <p className="mt-7 max-w-[464px] text-base leading-7 text-white/80">
            ...
            <Button
              asChild
              className="mt-8 h-10 w-[154px] rounded-xl bg-brand-orange px-7 font-inter text-base font-semibold text-white hover:bg-brand-orange/90"
            >
              <Link to="/vehicles">View all cars</Link>
            </Button>
          </div>
          <BookingForm className="relative z-10 w-full shadow-none lg:h-[503px] lg:w-[416px] lg:shrink-0" />
        </div>
      </div>
    </section>
  );
