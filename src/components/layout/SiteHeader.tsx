  ...
  return (
    ...
    >
      ...
      <span className="flex h-10 flex-col justify-center">
        <span className="text-[13px] leading-none text-black">Need help?</span>
        <span className="mt-1.5 text-[13px] font-semibold leading-none text-black">
          {helpPhone}
        </span>
      </span>
    </a>
        ...
        <nav className="hidden items-start gap-5 lg:flex" aria-label="Primary">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `px-3 py-1 font-inter text-lg ${isActive ? "font-bold text-black" : "font-medium text-black"}`
              }
            >
              {link.label}
            </NavLink>
